...
class A3View{
    ...
    private const VIEW_PERFIX = 'A3View_';
    private const VIEW_EXT = '.php';
    private $a3ViewUri;
    private $a3ViewHash = '';
    private $a3ViewData = [];
    private $a3ViewHeaders = [];
    private $mainExtends;
    ...
    private function getViewUri($uri){
    ...
    public function renderView(){
        if(self::isValidView($this->a3ViewUri,true) === false){
            ...
            $this->a3ViewHash = $this->getViewHash($this->a3ViewUri);
            $renderedView = $this->getRenderedViewUri($this->a3ViewHash);
            ...
            if(array_key_exists('__A3ViewExtends',$this->a3ViewData)){
                ...
                if($___A3ViewExtends !== false && $___A3ViewExtends instanceof A3ViewExtends){
    ...
    private function getViewHash($uri){
        return self::VIEW_PERFIX.md5($uri);
    }
    private function getRenderedViewUri($uri){
        return A3_ROOT_DIR.self::RENDERED_VIEWS_URI.'/'.$uri.self::VIEW_EXT;
    }
    private function replace($code){
        ...
        $code = preg_replace($this->getPattern(['A3','A3L','A3Words','A3Data','A3Date','A3GRS','A3Settings','A3Assest','A3Request','A3RequestFull']), "<?php print_r($1$2); ?>", $code);
    ...
    private function getPattern($search,$isEnd = false){
        if(is_array($search)){
            $search = implode('|',$search);
        }
        return '/@(' . $search . ( $isEnd ? ')\b/' : ')(\s*\(((?:[^\(\)]++|(?2))*)\))/' );
    }
    public static function isValidView($uri,$fullUri = false){
    ...
    public static function publishHeader($header){
        if(A3String::is($header)){
            header($header);
        }
    }
    private static function __error($text,$replace,$a3Function,$skip = false){
