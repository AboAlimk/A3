...
class A3RequestObject{
    ...
    private function repairLink($link){
        if(strpos(A3_DOMAIN.$link,A3_ROOT) !== false){
    ...
    public function setRequestRouteItems($data){
    ...
    public function getRouteItems($index = null){
    ...
    public function get($name = null,$def = false){
        if($name){
            return A3GetPathValue($_GET,$name,$def);
        }
        return $_GET;
    }
    public function post($name = null,$def = false){
        if($name){
    ...
    public function getRequestRouteItems($key = null){
        if($key === null){
            return $this->requestRouteItems;
        }
        return A3GetPathValue($this->requestRouteItems,$key,'');
    }
    public function getLink($replace = null){
        if($replace&&is_array($replace)){
            return A3RequestRoute::parse($this->getBaseRoute(), $replace);
        }
        return $this->getRoute();
    }
    public function getFullLink($replace = null){
        return A3_ROOT.$this->getRoute($replace);
    }
    ...
    public function __call($method, $parameters){
        self::__error('undefined_method','A3RequestObject::'.$method,__FUNCTION__);
    }
