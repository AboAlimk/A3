...
class A3DataBaseTableData{
    ...
    public function __construct($table_name){
        ...
        $this->charset = A3Settings('mysqli.database_charset');
    ...
    public function getData(){
        ...
        foreach($this->columns as $name=>$column){
            $data[] = $column->getData(strtolower($name) === strtolower($this->autoIncrement));
        }
        $text = "CREATE TABLE IF NOT EXISTS ".$this->table_name." (".implode(",",$data).")";
        if($this->charset){
