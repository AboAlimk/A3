...
class A3Error{
    private static $custom_error = false;
    private static $errorMessage = '';
    private static $errorFile = '';
    private static $errorLine = '';
    private static $errorSkip = false;
    ...
    public static function errorTrigger($data,$fire = true){
        self::$custom_error = $fire;
    ...
    private static function parseClass($aclass){
        if(is_string($aclass) && strpos($aclass,'\\') !== false){
            $arr = explode('\\',$aclass);
    ...
    private static function parseError($error){
        ...
        if(A3Settings("debug")===true){
            if(self::$custom_error){
                ...
                if(strpos($_error_message,'mysqli::__construct():')!== false){
                    if(strpos($_error_message,'php_network_getaddresses')!== false){
                    ...
                    }else if(strpos($_error_message,'access denied for user')!== false){
                        $text = self::getErrorData('mysqli_connection_data_error');
                        $text = A3String::replace($text,'%s','database username or password');
                        self::viewError($text,self::$errorFile,self::$errorLine);
                    }else if(strpos($_error_message,'unknown database')!== false){
                        $text = self::getErrorData('mysqli_connection_data_error');
    ...
    private static function viewError($error_message,$error_file,$error_line){
        $txt = '';
        ...
        $html = '<html>';
        $html .= '<head>';
        $html .= '<title>A3Error</title>';
        $html .= '<script>'.self::getErrorJS().'</script>';
        $html .= '<style>'.self::getErrorCSS().'</style>';
        $html .= '<style>ol.linenums>li:nth-child('.$error_line.'){background:#d43030;padding: 2px;}</style>';
        $html .= '<script>addEventListener("load", PR.prettyPrint, false);</script>';
        $html .= '</head>';
        $html .= '<body>';
        $html .= '<div class="cont">';
        $html .= '<div class="header-cont">';
        $html .= '<div class="header">A3Error</div>';
        if($error_message){
            $html .= '<div class="header-msg">'.$error_message.'</div>';
        }
        if(!self::$errorSkip && $error_file && $error_line){
            $html .= '<div class="header-file">'.$error_file.' <strong>LINE</strong> '.$error_line.'</div>';
        }
        $html .= '</div>';
        $html .= '<div class="body-cont">';
        if(!self::$errorSkip && $txt){
    ...
    private static function getErrorData($id){
        $errorText = [
            ...
            'a3request_middleprocess_error' => '<strong>A3Request:</strong> MiddleProcess should return A3RequestObject, A3Redirect, A3View or A3Response',
            ...
            'a3database_connction_error_method' => '<strong>A3DataBaseConnection:</strong> Call to undefined method "A3DataBaseConnection::<i>%s()</i>", use A3DataBaseConnection::new()',
    ...
    private static function getErrorJS(){
        return "
        !function(){
    ...
    private static function getErrorCSS(){
        return "
        body{
